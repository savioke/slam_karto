...
class SlamKarto
{
  ...
  private:
    ...
    std::string solver_type_, default_solver_type_;
    std::string visualizer_type_, default_visualizer_type_;
...
SlamKarto::SlamKarto() :
        ...
        mapper_(NULL),
        default_solver_type_("SPASolver"),
        default_visualizer_type_("SPAGraphVisualizer")
{
  ...
  if (!private_nh_.getParam("solver_type", solver_type_))
    solver_type_ = default_solver_type_;

  if (!private_nh_.getParam("visualizer_type", visualizer_type_))
    visualizer_type_ = default_visualizer_type_;
  ...
  std::stringstream solver_plugin_stream, default_solver_plugin_stream;
  solver_plugin_stream << "karto_plugins::" << solver_type_;
  default_solver_plugin_stream << "karto_plugins::" << default_solver_type_;
  try
  {
  ...
  {
    ROS_ERROR_STREAM(
        "The solver plugin " << solver_plugin_stream.str() << " failed to load. Error: " << ex.what()
            << ". We will try to load the default solver plugin instead: " << default_solver_plugin_stream.str());
    try
    {
      solver_ = solver_loader_.createInstance(default_solver_plugin_stream.str());
      ROS_INFO_STREAM("Loaded " << default_solver_plugin_stream.str() << " solver plugin");
    }
    catch (pluginlib::PluginlibException& ex)
    {
      ROS_ERROR_STREAM(
          "The default solver plugin " << default_solver_plugin_stream.str() << " failed to load. Error: " << ex.what()
  ...
  std::stringstream visualizer_plugin_stream,default_visualizer_plugin_stream;
  visualizer_plugin_stream << "karto_plugins::" << visualizer_type_;
  default_visualizer_plugin_stream << "karto_plugins::" << default_visualizer_type_;
  try
  {
  ...
  {
    ROS_ERROR_STREAM(
        "The visualizer plugin " << visualizer_plugin_stream.str() << " failed to load. Error: " << ex.what()
            << ". We will load the default visualizer plugin instead: " << default_visualizer_plugin_stream.str());
    try
    {
      visualizer_ = visualizer_loader_.createInstance(default_visualizer_plugin_stream.str());
      ROS_INFO_STREAM("Loaded " << default_visualizer_plugin_stream.str() << " visualizer plugin");
    }
    catch (pluginlib::PluginlibException& ex)
    {
      ROS_ERROR_STREAM(
          "The default visualizer plugin " << default_visualizer_plugin_stream.str() << " failed to load. Error: "
              << ex.what() << ". Shutting down the node.");
      ros::shutdown();
    }
  }
